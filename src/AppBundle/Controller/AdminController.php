...
class AdminController extends Controller
{
	...
	group by c.id";
        ...
        $sql = "select c.id, c.order_number, c.submit_date, sum(p.quantity) items, o.name as office_name, CONCAT_WS(\" \", c.requester_first_name, c.requester_last_name) as submitted_by, sum(p.back_order_quantity - p.back_order_ship_quantity) as bo_quan, (sum(p.back_order_ship_quantity)+sum(p.ship_quantity)) as bo_ship
	from cart c
		left join cart_products p
			on p.cart_id = c.id
		left join users u
    ...
    public function viewAllOutstandingOrdersAction(Request $request)
    {
        ...
        $sql = "
            select p.id, c.id as cart_id, p.quantity, c.approved, p.ship_quantity, 
                   p.returned_items_quantity, p.returned_items_shipped_quantity, 
                   parts.require_return, c.order_number, c.submit_date, c.approve_date, 
                   CONCAT_WS(\" \", c.requester_first_name, c.requester_last_name) as submitted_by, 
                   CONCAT_WS(\" \", u2.first_name, u2.last_name) as approved_by, o.name as office_name, 
                   COALESCE (parts.stock_number, p.stock_number) as stock_number, 
                   parts.description, p.description as description2
            from cart_products p
                left join cart c
                    on p.cart_id = c.id
                left join parts 
                ...
                left join offices o
                    on c.office_id = o.id
            where c.submitted = 1
            and (parts.require_return = 1 or p.return_required = 1)
            AND p.quantity > p.returned_items_quantity
        ...
        $sql = "select p.id, c.id as cart_id, c.approved, p.return_ship_date, p.return_received_date, p.quantity, p.ship_quantity, p.returned_items_quantity, p.returned_items_shipped_quantity, parts.require_return, c.order_number, c.submit_date, c.approve_date, CONCAT_WS(\" \", c.requester_first_name, c.requester_last_name) as submitted_by, CONCAT_WS(\" \", u2.first_name, u2.last_name) as approved_by, o.name as office_name, parts.stock_number, parts.description, p.description as description2
	from cart_products p
		left join cart c
			on p.cart_id = c.id
		left join parts 
